...
const obserwator = new IntersectionObserver(entries => {
	entries.forEach(entry => {
		if (entry.isIntersecting) {
			entry.target.classList.add('poka-itemy')
		} else {
			entry.target.classList.remove('poka-itemy')
		}
	})
})

const skalaSkrola = document.querySelectorAll(".skala-skrola");
skalaSkrola.forEach((el) => obserwator.observe(el));

const skrolDol = document.querySelectorAll(".skrol-dol");
skrolDol.forEach((el) => obserwator.observe(el));

const skrolGora = document.querySelectorAll(".skrol-gora");
skrolGora.forEach((el) => obserwator.observe(el));